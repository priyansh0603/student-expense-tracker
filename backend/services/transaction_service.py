...
def add_transaction(type_: str, amount: float, category: str, description: str, user_id: int, trans_date: str = None) -> dict:
    ...
    row = execute_query(
        ...
        (type_, amount, category, description, d, month, user_id),
        fetch='one'
    )
    return {"success": True, "id": row['id']}

def get_transactions(user_id: int, month: str = None, limit: int = None) -> list:
    """Get transactions, optionally filtered by month."""
    if not month:
        month = get_current_month()
    query = "SELECT * FROM transactions WHERE user_id = %s AND month = %s ORDER BY date DESC, created_at DESC"
...
def get_dashboard_data(user_id: int, month: str = None) -> dict:
    ...
    income_row = execute_query(
        "SELECT COALESCE(SUM(amount), 0) as total FROM transactions WHERE user_id = %s AND type='income' AND month=%s",
        (user_id, month), fetch='one'
    )
    expense_row = execute_query(
        "SELECT COALESCE(SUM(amount), 0) as total FROM transactions WHERE user_id = %s AND type='expense' AND month=%s",
        (user_id, month), fetch='one'
    )
    ...
    cat_rows = execute_query(
        """SELECT category, COALESCE(SUM(amount), 0) as total
           FROM transactions WHERE user_id = %s AND type='expense' AND month=%s
           GROUP BY category""",
        (user_id, month), fetch='all'
    )
    ...
    # Recent transactions
    recent = get_transactions(user_id, month, limit=8)

    # Friend totals
    from backend.services.friend_service import get_friend_totals
    friend_totals = get_friend_totals(user_id, month)
...
def delete_transaction(user_id: int, transaction_id: int) -> dict:
    execute_query("DELETE FROM transactions WHERE user_id = %s AND id = %s", (user_id, transaction_id))
    return {"success": True}
